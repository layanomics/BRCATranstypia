# app.py — FINAL guaranteed stable version
from pathlib import Path
import json, io, re, tempfile
import numpy as np
import pandas as pd
...
FEATURES_NORM = [norm_gene(g) for g in FEATURES_RAW]

def detect_id_system(names):
...
def auto_map_cols(cols, model_ids, uploaded_ids):
    out = []
    for c in cols:
        n = norm_gene(c)
...
def align_to_features(df, features_norm):
    ...
    for f in features_norm:
        if f not in out.columns:
            out[f] = np.nan
    return out[features_norm].apply(pd.to_numeric, errors="coerce").fillna(0.0)
...
with tab1:
    ...
    if uploaded:
        # Save to a temp file so Streamlit can re-read safely
        tmp_path = Path(tempfile.gettempdir()) / "upload.csv"
        ...
        try:
            ...
            model_ids, uploaded_ids = detect_id_system(FEATURES_NORM), detect_id_system(raw.columns)
...
with tab2:
    st.subheader("Paste one sample")
    head_preview = ",".join(FEATURES_NORM[:25]) + "..."
